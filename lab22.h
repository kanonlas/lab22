...
void Unit::equip(Equipment *stat){
if(equipment != 0){
vector <int> stat_o = equipment->getStat();
 hpmax -= stat_o[0];
 atk -= stat_o[1];
 def -= stat_o[2];
	if(hp > hpmax){
...
vector <int> stat_m = stat -> getStat();
hpmax += stat_m[0];
atk += stat_m[1];
 def += stat_m[2];
 equipment=stat;
...
Equipment::Equipment(int f,int g,int c){
hpmax = f; atk = g; def = c;
}
